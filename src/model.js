...
import {each,range,map,filter,meanBy} from "lodash-es"
...
const blind = Math.cos(( 180 - param.blindspot / 2 )/180*Math.PI);
...
const initialize = () => {
	...
	hideouts = map(range(param.number_hideouts), i => {
			const theta = Math.random() * Math.PI * 2;
			return {id:i, 
				x: Math.random() * L, 
				y: Math.random() * L, 
				vx: param.speed_hideouts * Math.cos(theta),
				vy: param.speed_hideouts * Math.sin(theta),
				r : param.size_hideouts
		}
	})	
...
const go  = () => {
	...
	each(agents,a=>{
		...
		colliders = filter(agents,d=>{
			return (dist(a,d) < param.collission_radius) && (d.id != a.id);
		})	
		...
		if(colliders.length>0) {		
			...
			const neighbors = filter(agents,d => {
				...
				return (d.r < param.attraction_radius.widget.value()  &&  d.id!=a.id)
			})
		...
		// now we separate them into the agents to align with and those to be attracted to
			...
			if (alignors.length > 0 && param.collective_behavior.widget.value()){
			...
			if (attractors.length > 0 && param.collective_behavior.widget.value()){
	...
	each(agents, d => {
		...
		const ishidden = filter(hideouts,a => dist(a,d)  < a.r ).length;

		const v = (ishidden == 0 || param.dark_zones.widget.value()==false) ? param.speed_in_the_light.widget.value() : param.speed_in_the_dark.widget.value();
...
export {agents,hideouts,initialize,go,update}
